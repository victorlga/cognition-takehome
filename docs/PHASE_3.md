...
Drive each seeded issue through the full kanban pipeline (Backlog → Planning → Building → Reviewing → Done) using the orchestrator and Devin sessions. This is an **orchestrator phase** that coordinates sub-phases.
...
1. `CHANGELOG.md` — read the entire file. Specifically:
   - PHASE_1 entry: issue numbers on the fork, project board number, status field node IDs
...
4. `docs/ARCHITECTURE.md` — prompt templates, state machine design
...
gh project list --owner victorlga
```
...
- [ ] Project board updated: all items in Done (or documented reason for skip)
...
5. The project board shows the issue in "Done"
...
- Project board reflects the final state
